...
conn.once('open', function ()
{
  ...
  function checkVotingPower (start = false, callback) {
    const limitPower = start ? 10000 : 6000;
  ...
  const VOTE_THRESHOLD=8;
    ...
    function sortContributions(){
        ...
        low_qual_post.forEach(function(post){post.real_vote=low_qual_post_vote;});
                                ...
                                .then(contributionsCount => {
                                  Post
                                    ...
                                    .then(contributions => {
                                     ...
                                     var cat=post.json_metadata.type.replace('announcement-',''); //put announcements with their corresponding category
                                     if(cat==='graphics'||cat==='documentation'||cat==='analysis'||cat==='social')
                                         cat='others'; //regroups the categories with low amount of contributions
                                     //console.log(i,post.author,post.permlink,vote, post.json_metadata.type);
